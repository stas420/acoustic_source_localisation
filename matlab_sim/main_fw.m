...
source_pos = [3; 2.5; 2.5]; % przykładowa pozycja źródła
...
% title('sig');
% subplot(2, 2, 2);
% plot(ura_rx_rev_noise_low);
% title('ura rx rev noise low');
% subplot(2, 2, 3);
% plot(f_axis, abs(fft(sig)));
% title('fft(sig)');
% subplot(2, 2, 4);
% plot(f_axis, abs(fft(ura_rx_rev_noise_low)));
...
%% -- MUSIC -- << almost done, almost working - angles conv to be checked
lambda_0 = C/f_0;
